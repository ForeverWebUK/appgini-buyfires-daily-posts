...
ACCESSORY_IMAGE_RE = re.compile(r"\b(remote|app)\b", re.IGNORECASE)


def compact_product(p):
    # Drop remote-control / app-screenshot shots (identifiable from alt
    # text) before capping at 4, so a genuine in-situ or hero shot isn't
    # squeezed out by an accessory close-up. Whether a remaining image is
    # a plain white-background studio shot vs a real in-situ photo isn't
    # something the API data can tell us: that needs an actual look at
    # the images each run, not just this filter.
    images = [
        {"src": im.get("src"), "alt": im.get("alt") or ""}
        for im in (p.get("images") or [])
        if not ACCESSORY_IMAGE_RE.search(im.get("alt") or "")
    ][:4]
